...
export default function EditNewsletter() {
  ...
  useEffect(() => {
    const fetchNewsletter = async () => {
      try {
        ...
        const data = res.data;
        // Fix date formats for input types
        if (data.events) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://127.0.0.1:5000/api/newsletters/${id}`, formData, {
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #e8eeff 0%, #f0f4ff 50%, #eef2ff 100%)' }}>
      <div className="min-h-screen pb-24">
        ...
        <div className="bg-white/80 backdrop-blur-2xl border-b border-white/50 sticky top-0 z-20 px-6 py-4 flex justify-between items-center shadow-sm">
          <div>
            <h1 className="text-xl font-black font-serif text-gray-900">Edit Newsletter</h1>
            <p className="text-xs text-gray-400 font-medium mt-0.5">Update and republish</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={handleUpdate} disabled={loading} className="btn-primary flex items-center gap-2">
              <Save size={18} /> {loading ? 'Saving…' : 'Save Updates'}
            </button>
          </div>
        </div>
